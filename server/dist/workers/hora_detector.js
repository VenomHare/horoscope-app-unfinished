...
async function getSunriseTime() {
    ...
    const sunTime = await fetch(`https://api.sunrisesunset.io/json?lat=${lat}&lng=${lng}`);
    const data = await sunTime.json();
    const suntime = new Date(`${new Date().toISOString().split("T")[0]} ${data.results.sunrise}`);
    return suntime;
}
export async function getCurrentHora(now = new Date()) {
    ...
    const currentHoraIndex = ((currentHour - sunriseHour + dayIndex * 24) % Graha_sequence.length) -
        (hashoraEnded ? 0 : 1);
    console.log(currentHoraIndex);
    console.log(currentHour, sunriseHour, dayIndex, Graha_sequence.length, hashoraEnded);
    //   console.log(
    //     chalk.cyan(
    ...
    return {
        ...
        endTime: `${hashoraEnded ? currentHour + 1 : currentHour}:${sunriseMinutes}`,
    };
}
//# sourceMappingURL=hora_detector.js.map